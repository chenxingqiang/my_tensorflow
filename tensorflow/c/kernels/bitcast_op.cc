...
static void BitcastOp_Compute(void* kernel, TF_OpKernelContext* ctx){
    ...
    dim_count = TF_NumDims(tensors);
        if (!(k->insize >= k->out_size || (dim_count> 0 &&
    ...
    TF_Tensor* output = TF_AllocateTensor(k->output_data_type, dims, 0, TF_DataTypeSize(k->output_data_type));
...
void RegisterBitcastOpKernel(){
    TF_Status* status = TF_NewStatus();
    {
        auto* builder = TF_NewKernelBuilder("Bitcast", tensorflow::DEVICE_CPU,
                                             ...
                                             &BitcastOp_Delete);
        TF_RegisterKernelBuilder("Bitcast", builder, status);
