...
#include "tensorfloiw/core/platform/types.h"
...
struct TFStatusDeleter {
    void operator()(TF_Status* tf_status) const { TF_DeleteTensor(tf_status);}
...
using Safe_TF_StatusPtr = std::unique_prt<TF_Status, TF_StatusDeleter>;
...
void* HistogramSummaryOp_Create(TF_OpKernelConstruction* ctx){
    HistogramSummaryOp* kernel = ne HistogramSummaryOp;
...
void HistogramSummaryOp_Compute(void* kernel, TF_OpKernelContext* ctx){
    ...
    tensorflow::tstring* output_tstring = reinterpret_cast<tensorflow::tstring*>(TF_TensorData(summary_tensor.get()));
...
void RegisterHistogramSummaryOpKernel() {
    ...
    {
    auto* builder = TF_NewKernelBuilder("HistogramSummary",tensorflow::DEVICE_CPU, &HistogramSummaryOp_Create,
    ...
    CHECK_EQ(TF_OK, TF_GetCode(status))
        << "Error while registering Histogram Summary kernel";
    }
