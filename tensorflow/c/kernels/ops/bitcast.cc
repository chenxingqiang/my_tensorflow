...
static void ComputeNewShape(TF_ShapeInferenceContext* ctx,TF_ShapeHandle* shape, TF_DataType input_type,
                ...
                if (TF_GetCode(status) == TF_OK) {
                   TF_DimensionHandle* last_dim = TF_NewDimensionHandle();
                    ...
                    if (!TF_DimensionHandleValueKnown(last_dim) ||
                           TF_DimensionHandleValueKnown(last_dim) == divisor_val){
                    ...
                    TF_ShapeInferenceContextConcatenateShapes(ctx, shape, extension, shape, status);
                    TF_DeleteDimensionHandle(extension);
                    }
            }
...
void RegisterBitcastOp() {
     ...
     TF_OpDefinitionBuilderSetShapeInferenceFunction(
        op_builder,&bitcast_shape_inference_fn
     );
