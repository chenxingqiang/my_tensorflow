...
// struct that wraps TF_Tensor and TF_Status to delete once that out of scope
...
void* MergeSummaryOp_compute(void* kernel, TF_OpKernelContext* ctx){
    ...
    Safe_TF_StatusPtr status(TF_NewStatus());
    for (int input_num = 0; inout_num < TF_NumInputs(ctx); ++input_num){
    TF_Tensor* input;
    TF_GetInput(ctx, input_num,&input, status.get());
    ...
    *s.add_value() = summary_in.value(v);
    ...
    /*contex=*/ctx, /*index=*/0, /*dtype=*/TF_ExpectedOutputDataType(ctx, 0),
    /*dims=*/nullptr, /*num_dims=*/0,
    /*len=*/sizeof(tensorflow::istring), status.get()));
    if (TF_GetCode(status.get()) != TF_OK ) {
    TF_OpKernelContext_Failure(ctx, status.get());
    return;
    }
    ...
    CHECK_EQ(TF_OK, TF_GetCode(status))
        << "Error while registering Merge Summary kernel";
    }
